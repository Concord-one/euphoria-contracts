...
contract Treasury is Policy {
    ...
    address public sConcord;
    uint256 public sConcordQueue; // Delays change to sOHM address
    ...
    constructor(
        ...
        /*isLiquidityToken[_CCRDDAI] = true;
        liquidityTokens.push(_CCRDDAI);*/
    ...
    function deposit(
        ...
        IERC20Mintable(CCRD).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOfToken(_token, _amount);
        ICCRDERC20(CCRD).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sConcord).balanceOf(msg.sender); // Can only borrow against sOHM held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithCCRD(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        ICCRDERC20(CCRD).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, CCRD, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(CCRD).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(CCRD).totalSupply().sub(totalDebt));
    }
    ...
    function valueOfToken(address _token, uint256 _amount) public view returns (uint256 value_) {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(CCRD).decimals()).div(10**IERC20(_token).decimals());
    ...
    function queue(MANAGING _managing, address _address) external onlyPolicy returns (bool) {
        ...
        } else if (_managing == MANAGING.sConcord) {
            // 9
            sConcordQueue = uint32(block.timestamp).add32(secondsNeededForQueue);
        } else return false;
